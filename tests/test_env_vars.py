...
def test_get_kernel_display_limit_none(monkeypatch):
    ...
    monkeypatch.delenv(env.DISPLAY_LIMIT_ENV_NAME, raising=False)
    assert env.get_kernel_display_limit() == env.DEFAULT_DISPLAY_LIMIT
...
def test_get_kernel_display_limit_wrong(monkeypatch):
    ...
    monkeypatch.setenv(env.DISPLAY_LIMIT_ENV_NAME, 'not_an_integer')
    with pytest.raises(ValueError):
        env.get_kernel_display_limit()
...
def test_get_module_sorting_none(monkeypatch):
    ...
    assert env.get_module_sorting() == env.ModuleSorting(env.DEFAULT_MODULE_SORTING)

def test_get_module_sorting_invalid(monkeypatch):
    """Test get_module_sorting raises a ValueError when the environment variable is set to an invalid value."""
    monkeypatch.setenv(env.MODULE_SORTING_ENV_NAME, 'invalid_value')
    with pytest.raises(ValueError):
        env.get_module_sorting()

def test_get_module_sorting_valid(monkeypatch):
    """Test get_module_sorting returns the correct value when the environment variable is set to a valid value."""
    for value in [_.value for _ in env.ModuleSorting]:
        monkeypatch.setenv(env.MODULE_SORTING_ENV_NAME, value)
        assert env.get_module_sorting() == env.ModuleSorting(value)
...
async def test_kernel_display_limit_sorting_desc_integration(
    ...
    monkeypatch.setenv(env.DISPLAY_LIMIT_ENV_NAME, '1')
...
async def test_kernel_display_limit_sorting_asc_integration(
        monkeypatch, jp_fetch, jupyter_server_module1, jupyter_server_module2
    ):
    """Integration test display_limit environment variable: checks that the launcher_version takes precedence."""
    monkeypatch.setenv(env.DISPLAY_LIMIT_ENV_NAME, '1')
    monkeypatch.setenv(env.MODULE_SORTING_ENV_NAME, 'asc')
...
async def test_kernel_display_limit_soring_desc_integration2(
        monkeypatch, jp_fetch, jupyter_server_module2, jupyter_server_module3
    ):
...
async def test_kernel_display_limit_availsorting_integration1(
        monkeypatch, jp_fetch, jupyter_server_module3, jupyter_server_module1
    ):
